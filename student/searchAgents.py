    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if (not hitsWall):
                ...
                tempVisitedCorners = list(visitedCorners)
                if nextState in self.corners:
                    if nextState not in tempVisitedCorners:
                        tempVisitedCorners.append(nextState)
                successor = ((nextState, tempVisitedCorners), action, 1)
...
def foodHeuristic(state, problem):
    ...
    for food in foodGridList:
        mlist.append(manhattan(food, position))
